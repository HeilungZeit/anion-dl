 ...
 *
 * Переехала сюда целиком со страницы аниме, когда та разошлась на вкладки.
 * Ничего не потеряла: озвучка, качество, папка, раскладка по подпапкам, выбор
 * серий и полоса очереди — всё на месте.
 */
@Component({
  ...
  styleUrl: './downloads-tab.component.scss',
...
export class DownloadsTabComponent {
   ...
   * «Очистить завершённые» задач нет, а файлы есть, и без этой сверки вкладка
  ...
  private readonly taskById = computed(
  ...
  readonly queuedCount = computed(() => {
    const own = new Set(this.videos().map((video) => `${video.videoId}`));
  ...
  private async withoutRedundant(
    ...
    const rest = episodes.filter((episode) => !downloaded.has(episode.videoId));
  ...
  percent(episode: Video): number {
    ...
    return Math.min(
      100,
      Math.round((task.processedSecs / task.totalSecs) * 100)
    );
  }
